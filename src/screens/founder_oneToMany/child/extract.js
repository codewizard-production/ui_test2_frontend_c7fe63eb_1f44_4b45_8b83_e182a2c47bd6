...
import FounderJsonConfig from "config/founder_product_onetomany_oneToMany_config.json";
...
const MapItems = [
    ...
    { navpropname: "", uicomponent: "founder", expand: "", exclude: [], func: Support.AddOrUpdateFounder }
];

const FetchFounderInfo = async () => {
    let item = {};
    return new Promise(async (resolve) => {
        const keyItems = Object.keys(FounderJsonConfig);
        keyItems.forEach(elm => {
            let items = [];
            for (let prop of FounderJsonConfig[elm]) {
...
const FetchFounderDetails = async (founderId, enums) => {
    ...
    return new Promise(async (resolve) => {
        ...
        if (founderId) {
            ...
            rslt = await Api.GetFounderSingle(founderId, `$expand=${$expand}`);

            if (rslt.status) {

                const founder = rslt.values;
                ...
                for (let i = 0; i < MapItems.length; i++) {
                    ...
                    const sourceObj = Helper.IsNullValue(source) ? founder : founder[source];
                    if (parent && child) {
                        let _tmpItems = [];
...
const Extract = async (founderId) => {

    return new Promise(async (resolve) => {

        let rtnObj = { row: {}, options: [], backRow: {} };

        await FetchFounderInfo().then(async (item) => {
            ...
            await FetchDropdownItems(items).then(async (enums) => {
                ...
                if (!Helper.IsNullValue(founderId)) {
                    await FetchFounderDetails(founderId, enums).then(({ row, backRow, mapitems }) => {
