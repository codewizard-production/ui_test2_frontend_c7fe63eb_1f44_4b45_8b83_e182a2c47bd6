...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, prodImages, founderId, changes, numfields;
        const mapItems = MapItems;

        // Attach inline objects
        let founder = row['founder'];
        founderId = row['founder'].find((x) => x.type === 'keyid').value;
        ...
        inlineObjs.forEach(x => {
            ...
            founder.push({ key: x.property, value: vObj, type: "inline" });
        });
        ...
        // Add Or Update Founder
        changes = TrackChanges('founder');
        if (changes.length > 0) {
            numfields = Helper.GetAllNumberFields(founder);
            if (numfields.length > 0) Helper.UpdateNumberFields(founder, numfields);
            rslt = await Support.AddOrUpdateFounder(founder, []);
            if (rslt.status) {
                founderId = rslt.id;
            } else { return; }
        }
        ...
        for (let i = 0; i < mapItems.length; i++) {
            ...
            if (changes.length > 0) {
                // Add or Update the Founder and navigation entity if it is deos not exist
                let navItem = founder.find(x => x.uicomponent === mapItems[i].uicomponent);
                if (!Helper.IsJSONEmpty(navItem) && Helper.IsNullValue(navItem.value)) {
                    ...
                    if (rslt.status) {
                        data = [
                            { key: "FId", value: parseInt(founderId) },
                            { key: navItem.key, value: parseInt(rslt.id) }
                        ];
                        rslt = await Support.AddOrUpdateFounder(data);
        ...
        global.AlertPopup("success", "Founder is created successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
