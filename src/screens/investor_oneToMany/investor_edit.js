...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, prodImages, investorId, changes, numfields;
        const mapItems = MapItems;

        // Attach inline objects
        let investor = row['investor'];
        investorId = row['investor'].find((x) => x.type === 'keyid').value;
        ...
        inlineObjs.forEach(x => {
            ...
            investor.push({ key: x.property, value: vObj, type: "inline" });
        });
        ...
        changes = TrackChanges('investor');
        if (changes.length > 0) {
            numfields = Helper.GetAllNumberFields(investor);
            if (numfields.length > 0) Helper.UpdateNumberFields(investor, numfields);
            rslt = await Support.AddOrUpdateInvestor(investor, []);
            if (rslt.status) {
                investorId = rslt.id;
            } else { return; }
        }
        ...
        for (let i = 0; i < mapItems.length; i++) {
            ...
            if (changes.length > 0) {
                // Add or Update the Investor and navigation entity if it is deos not exist
                let navItem = investor.find(x => x.uicomponent === mapItems[i].uicomponent);
                if (!Helper.IsJSONEmpty(navItem) && Helper.IsNullValue(navItem.value)) {
                    ...
                    if (rslt.status) {
                        data = [
                            { key: "InvestorId", value: parseInt(investorId) },
                            { key: navItem.key, value: parseInt(rslt.id) }
                        ];
                        rslt = await Support.AddOrUpdateInvestor(data);
        ...
        global.AlertPopup("success", "Investor is created successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
