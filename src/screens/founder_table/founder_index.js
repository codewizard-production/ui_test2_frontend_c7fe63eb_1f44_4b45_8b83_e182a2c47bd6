...
const columns = [
    { headerName: "FId", field: "FId", flex: 1 },
    { headerName: "Name", field: "Name", flex: 1 },
    { headerName: "Role", field: "Role", flex: 1 },
    { headerName: "Bio", field: "Bio", flex: 1 },
    { headerName: "ContactNumber", field: "ContactNumber", flex: 1 },
    { headerName: "LinkedInProfile", field: "LinkedInProfile", flex: 1 },
];
...
const Component = (props) => {
    ...
    const FetchResults = async () => {
        ...
        await Api.GetFoundersCount(query)
            .then(async (res) => {
                if (res.status) {
        ...
        await Api.GetFoundersMulti(query, "")
            .then(async (res) => {
                if (res.status) {
                    _rows = res.values || [];
                    if (_rows.length > 0) {
    ...
    const OnCloseClicked = async (e) => {
        if (e) {
            const rslt = await Api.SetFounderSingle({ FId: deletedId, Deleted: true });
            if (rslt.status) {
                setInitialize(true);
    ...
    const OnActionClicked = (id, type) => {
        let _route;
        if (type === 'edit') _route = `/Founders/edit/${id}`;
        if (type === 'view') _route = `/Founders/view/${id}`;
    ...
    return (
        ...
        <>
            <Container {...props}>
                <Box style={{ width: '100%', paddingBottom: 5 }}>
                    ...
                    <Stack direction="row">
                        <Grid container sx={{ justifyContent: 'flex-end' }}>
                                ...
                                onClick={() => NavigateTo("/Founders/create")}
                            >
                                <AddBoxIcon />
                            </IconButton>
                        </Grid>
                ...
                <Box style={{ width: '100%' }}>
                    <DataTable keyId={'FId'} columns={columns} rowsCount={rowsCount} rows={rows} sortBy={sortBy} pageInfo={pageInfo} onActionClicked={OnActionClicked}
